...
three 2026-09-12/13 rounds (see CLAUDE.md's "ADMIN DASHBOARD REBUILD", "COACH DASHBOARD
REBUILD", and "2026-09-13 CLIENT FEEDBACK" sections for the full story) — Salary, Coach
Swapping, Chat, Compliance, the standalone Reports/Analytics section, GPS/selfie-verified
attendance, and the admin Activities/Batches UI are all gone system-wide; do not re-add any
of them without an explicit decision to do so.
...
frontend/   React admin dashboard (students, coaches, attendance incl. approve/reject,
            fees, leave, settings) plus a coach web dashboard (students, classes,
            attendance incl. approval status, leave, fee receipts/reminders, settings)
...
- **CLAUDE.md** — the living spec: full requirements, API reference, and a dated history of
  every real bug found and fixed (root cause + what changed), including the three 2026-09-12/13
  dashboard-rebuild rounds. Read this before touching auth, notifications, photo storage, or
  the attendance-approval/leave workflow — each has non-obvious history worth knowing first.
