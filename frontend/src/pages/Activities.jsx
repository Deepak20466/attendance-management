      ...
      {loading || !roster ? (
        ...
        <>
          ...
          <div style={{ display: "flex", gap: 16, marginBottom: 12, fontSize: "0.85rem" }}>
            ...
            <span>Not Confirm: <strong>{roster.not_confirm_count}</strong></span>
                    ...
                    <td>
                      <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                        {STATUS_OPTIONS.map((opt) => (
                          ...
                          >
                            {opt === "NOT_CONFIRM" ? "Not Confirm" : opt.charAt(0) + opt.slice(1).toLowerCase()}
                          </button>
                        ))}
                      </div>
                    </td>
...
function ActivityManageModal({ activity, onClose }) {
  ...
  const viewGroupPhoto = async (c) => {
    setGroupPhotoFor(c);
    setGroupPhotoUrl(null);
    setGroupPhotoLoading(true);
    try {
      const { data } = await ActivitiesAPI.groupPhotoBlob(c.id);
      setGroupPhotoUrl(URL.createObjectURL(data));
    } catch (err) {
      toast.error(err.response?.data?.detail || "Failed to load group photo");
    } finally {
      setGroupPhotoLoading(false);
    }
  };

  const closeGroupPhoto = () => {
    setGroupPhotoFor(null);
    setGroupPhotoUrl(null);
  };
  ...
  return (
            ...
            <tbody>
              {classes.map((c) => (
                <tr key={c.id}>
                  ...
                  <td className="table-actions">
                    {c.has_group_photo && (
                      <button className="btn btn-secondary btn-sm" onClick={() => viewGroupPhoto(c)}>
                        View Group Photo
                      </button>
                    )}
    ...
    {groupPhotoFor && (
      <Modal title={`Group Photo — ${groupPhotoFor.date}`} onClose={closeGroupPhoto}>
        {groupPhotoLoading ? (
          <div className="empty-state">Loading...</div>
        ) : groupPhotoUrl ? (
          <img src={groupPhotoUrl} alt={`Group photo, ${groupPhotoFor.date}`} style={{ width: "100%", maxWidth: 480, display: "block", margin: "0 auto", borderRadius: 8 }} />
        ) : (
          <div className="empty-state">Failed to load photo.</div>
        )}
      </Modal>
    )}
    </>
